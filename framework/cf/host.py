...
class HostContext:
    ...
    async def llm_generate(self, *, chat_provider_id: str, prompt: str, **kwargs: Any) -> Any:
        ...
        ``system_prompt`` is forwarded rather than dropped: the shipped adapter passes
        it only when it has one to pass, and "did the host persona reach a proactive
        render?" is exactly the question this harness exists to answer.
        """
...
class AstrBotHost:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            system_prompt: What AstrBot would assemble as the system prompt for a
                normal turn (persona + skills + tool reminder). It is handed to the
                plugin on every chat turn, exactly like the real host does, so the
                adapter's caching of it can be exercised.
        """
    ...
    def user_turn(
        ...
        *,
        ...
        with self._turn_lock:
            ...
            # The real pipeline hands the plugin a ProviderRequest whose system_prompt
            # AstrBot has already assembled; a render call never gets one at all.
            request = ProviderRequest(prompt=text, system_prompt=self.system_prompt)
