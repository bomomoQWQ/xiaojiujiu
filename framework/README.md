...
| 对话活不过一次重启；`/session work` 两个会话分不出来 | **持久对话记录**：每次可见的对话追加进 JSONL，重开窗口回显本会话最近几条，`/history [n]` 重看，`readline` 的上翻历史按会话恢复 |
...
```toml
[harness]
history = "runs/chat_history.jsonl"   # 相对配置文件解析；留空 = 放在配置文件旁边
recap = 6                             # 打开窗口时回显本会话最近几条；0 = 不回显
```

```bash
cf chat --history /tmp/my.jsonl --recap 0     # 命令行覆盖
# 窗口里： /history 20  重看本会话最近 20 条
```

- **不放进 `run_dir`**：`run_dir` 带时间戳、每次换一个，记录活不过重启就不叫记录。
  默认放在配置文件旁边，让一个实验带着自己那段对话走。
- **格式**：一行一个 JSON（`at` / `kind` / `session` / `text`），与 `trace.jsonl` 同样的形状，
  可以直接 `tail` / `grep`。坏行**跳过并计数**（`/history` 与启动回显都会说有几行读不出来），
  不会让窗口起不来。
- **只记可见的三类**：用户说的、角色回的、主动发来的。**运行时的隐藏背景块永远不写进来**
  （设计 §2.6 / §86.10：隐藏心理上下文按轮注入、用完即弃）。这不是靠"记得别传"——
  `cf/history.py` 的 `record()` 对白名单以外的 `kind` **直接抛错**，因为这种写入一旦发生是
  **静默**的：文件里多了一段不该有的东西，没有任何症状。
- 一次回复只会被记一条：流式打印与投递回调看到的是同一段文本，两者都写就会让每个回答翻倍。
...
"$PY" -m pytest tests          # 293 passed（约 50s）
```
...
| `cf/cli.py` | `cf` 命令 |
| `cf/history.py` | 聊天窗口的持久对话记录（JSONL，只记可见的三类：用户 / 回复 / 主动） |
| `tests/` | 293 个测试 |
