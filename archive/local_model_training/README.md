...
> ## ⚠️ 架构补丁 v0.2 之后的定位变化
>
> 请先读 `../PATCH_v0.2_即时演出与持久认知分离_移除本地2B核心依赖.md`。
>
> **本工程训练出的模型已不再是 Runtime 的标准依赖。** 补丁把系统拆成两层：
>
> * **即时演出层**——宿主主 LLM 直接理解当前用户说的话并当场表现。这一层不需要本模型。
> * **持久认知层**——Runtime 负责跨时间状态连续性，默认只用规则/统计，`SemanticProvider`
>   默认是 `disabled`，Runtime 在没有本模型时功能完整。
>
> 因此本工程现在的定位是**可选加速器**：产出物通过 `LocalCPUProvider` / `LocalGPUProvider`
> 接入 Runtime 的**低频深层认知刷新**（重解释旧事件、心理状态语言化），
> 而不是每轮即时评价。
>
> 实测 CPU 成本（i7-13700H，Q4_K_M）：8 线程约 20 tok/s、单核约 8.9 tok/s、
> 半核约 3.7 tok/s，常驻约 2 GB。这个量级决定了它只能做异步低频任务，
> 不能挡在任何用户可见的路径前面——这正是补丁移除它标准地位的直接原因。
>
> 工程质量与数据管线（schema、不变量、生成、校验、评测）本身继续有效，
> 它们是任何语义 Provider 的质量基线。

---

## 0. 这个模型到底做什么

模型在 Runtime 里只承担两件事，都来自架构文档：
...
* **§8.1** 事件评价**不输出最终情绪值**。模型只回答"这件事是什么性质"，
