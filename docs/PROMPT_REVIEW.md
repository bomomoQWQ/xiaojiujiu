...
| **B** | 主动消息渲染 | Runtime outbox → `render` 行动 → `llm_generate` | 我们的块（去掉"我想做的"行）+ 【我现在要说的话】+ 意图/为的是什么/约束 | ❌ **完全没有** | ⚠️ 只有我们的，且错 | ❌ | ❌ |
...
> **这一节是 2026-09-18 的实测快照，保留原样。** 之后再改过两处：
> ① 下面那条 `当前本地时间`（值却是 UTC）已修，块里现在**不再有任何钟**，绝对时间只在主动
> 消息的渲染那一段陈述一次；
> ② 块的措辞换成了第一人称口语（前言、七个节名、心理标签、局势标签、意图行、注意行、收尾句）。
> **当前的逐字文本与真实样例在 `runtime/docs/PROMPTS_TO_THE_MAIN_LLM.md`**，以那份为准。
...
| 注入内容 | 位置 | 落到哪 | 文本 | 生效条件 |
|---|---|---|---|---|
| Runtime 上下文块 | 插件 `main.py`（`_inject_context`，由 `on_llm_request` 调） | `req.extra_user_content_parts`，用 `TextPart(text=…).mark_as_temp()` 追加 | Runtime `POST /context/render-block` 的整段（617 字，逐字见 §1.4） | ✅ `inject_enabled=True`；Runtime 不可达或缺 `TextPart`/`mark_as_temp` 时**放弃注入**（fail-open） |
| 主动消息的 prompt | Runtime `api_v1.py:597-649`（`_render_payload`）→ 插件 `astrbot_executor.py:52-94`（`render`）→ `context.llm_generate(prompt=…)` | **一次性生成调用**（不走 agent 流水线） | 块（去掉 `- 我想做的：` 行）+【我现在要说的话】+ 我想做的/为的是/约束 + 输出要求 | ✅；`system_prompt=""` ✗（见 §2） |
