...
**首次使用（728260403）**：8 条，四个 kind 各 2 条 —— 称谓 / 未决的现实关系 / 惦记"被忘记"
/ 未来迷茫 / 自责归因 / 收尾习惯 / 图片互动 / 对 AI 性质的好奇。
清单文件在 **`/home/bomomo/astrbot_test/carryover/728260403-20260918.json`，故意不入库**：
...
**其余 7 人的审计结论**（同一套脚本，排除用户点名的 `1670681411` 与 `3640344731`=机器人自己的号）：

| 账号 | 真人消息 | 值得留 | 不值得留 |
|---|---|---|---|
| `1070754640`「星河Sarit（sleeping）」 | 10 | 称谓；互动风格（要反问、反感单向输出与答非所问，不满直接用「？」）；两个**从没回答过**的问题：「你为什么叫苏清徽」「你是做什么的」 | 43 条记忆里约 40 条是同样 4 件事的复述 |
| `1913447173` | 31 | 已有 2 条好 durable（期待双向靠近「为啥不能你找我」；关心关系定位但用「奥～」自己带过）；风格：极简收尾「好的。」「。」、玩闹式悬念「不告诉你」、赌气后仍会回来、开场「你在干嘛」 | 67 条里约 60 条同义复述 |
| `2259606745`「硬邦面包」 | 15 | 称谓+来源群（✨AstrBot 1群✨）；**炒股**（两次问创业板）；两次要求唱歌；「#nepcat」暗号含义未明 | 41 条几乎全是这几件事的复述 |
| `994959351` | 15（另 2760 条自动回复） | 对**回应形式**极敏感（先要去掉句号、再去掉换行；他判断"是不是真人"看形式有没有被真正改动）；焦躁时用「急」「那行」自己降温（别追问）；「谁先开口」的困惑 | 66 条里约 60 条是模型对「[自动回复]。」间隔的**过度解读**；候选 **800 条** ✗ |
| `2206929446` | 1（「我是。」） | 无 | 无 |
| `20001` | — | 它是**假用户实例**（我自己的调试消息；那 3 条 boundary 是 red-team 测试留下的） | 全部 |

**注意 `2259606745` 的「今天是什么日子」**：模型当时的理解是"他在试探我是否记得某个对他有意义的
...
| 账号 | 条数 | 内容 |
|---|---|---|
| `1913447173` | 6 | 双向靠近（「为啥不能你找我」）/ 关系定位问了又自己带过（「我们是什么关系」→「奥～」）/ 玩闹式拒绝要顺着接住（「不告诉你」「不想」「朋友都多了」）/ 极简说话风格（开场「你在干嘛」、收尾「好的」「。」，赌气后自己回来）/ 喜欢教学式互动（「教你挺好的」）/「你叫什么」是个来回梗 + 「好巧啊，我也是」制造对等 |
| `1070754640` | 5 | 自称「星河Sarit（sleeping）」/ 要反问、反感单向输出与答非所问 / 那轮碎片化试探（「？」「1」「2」「🤔」→「不错，反问挺自然」）/ 两个**没被正面回答过**的问题：名字由来、你是做什么的 |
...
**顺带发现一个缺陷（没修，记下来）**：`1913447173` 的未决事项「等待用户告知结果」挂在他的
...
下次重建那个端口就换人了。今天 **8795 = `3309892640`（小钟）**；谁在哪儿看 `/fleet/status`
...
**重建两次（22:03、22:04）没有弄丢任何消息** ✓：没有 5xx/超时，728260403 那轮对话完整
...
| `1070754640` | +0.136 | 09-19 08:32 | 09-19 22:22 | 5 条候选（搬运进去的记忆都成了"主动提起"型候选） |
| `1913447173` | +0.079 | 09-19 09:42 | 09-20 02:32 | 3 条候选 |
| `728260403` | −99（冷却哨兵） | 09-19 11:42 | 09-20 09:22 | 刚聊过（冷却）→ 排到上午 |
| `1670681411` / `2206929446` / `2259606745` / `994959351` | −0.084 | 09-19 13:47 | 09-20 15:27 | 只有"无缘无故打个招呼"一条候选（空白库，状态完全一致） |
| `3309892640`（小钟） | −0.064 | 09-19 13:52 | 09-20 15:52 | 同上，库更年轻、缺席时钟刚起步 |
...
**顺带核实的**：deep refresh 是通的（728260403 在 22:07 跑了一次 `ran=1 degraded=0
