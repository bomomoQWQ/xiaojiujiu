...
class CandidateProjection:
    ...
    def upsert(self, connection: sqlite3.Connection, candidate: CandidateIntent) -> str:
        ...
        connection.execute(
            ...
            "unfinished_relevance, emotion_relevance, created_at, updated_at, wording_at, expires_at, "
            "retired_reason, proposed_by) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) "
            ...
            "wording_at=excluded.wording_at, expires_at=excluded.expires_at, "
            ...
            (
                ...
                isoformat(candidate.created_at or utcnow()),
                now,
                isoformat(candidate.wording_at or candidate.created_at or utcnow()),
    ...
    def _to_candidate(row: sqlite3.Row) -> CandidateIntent:
        ...
        return CandidateIntent(
            ...
            created_at=parse_datetime(data.get("created_at")),
            updated_at=parse_datetime(data.get("updated_at")),
            # Old rows predate the column; ``created_at`` is the right answer for them
            # (nothing could rewrite the wording back then without also resetting the row).
            wording_at=parse_datetime(data.get("wording_at")) or parse_datetime(data.get("created_at")),
            expires_at=parse_datetime(data.get("expires_at")),
