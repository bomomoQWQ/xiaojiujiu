...
class Runtime:
    ...
    def _recently_spoken_memory_ids(self, *, now: datetime) -> set[str]:
        ...
        ``candidate.memory_callback_cooldown_hours`` is about *material*: a memory that
        has been the opening line recently must not become the opening line again. The
        candidate itself records which memory it came from
        (``memory:<memory_id>`` in ``sources``), so a sent attempt is enough to know.

        Args:
            now: Reference time.

        Returns:
            Memory ids used since ``now - cooldown``. Empty when the guard is off.
        """
        hours = float(
            getattr(self.config.candidate, "memory_callback_cooldown_hours", 0.0) or 0.0
        )
        if hours <= 0.0:
            return set()
        cutoff = now - timedelta(hours=hours)
        prefix = candidate_module.MEMORY_SOURCE_PREFIX
        used: set[str] = set()
        for attempt in self.projections.attempts.list_by_state(
            [AttemptState.SENT.value], limit=ATTRIBUTION_SCAN_LIMIT, newest_first=True
        ):
            sent_at = ensure_aware(attempt.committed_at or attempt.created_at)
            if sent_at is None or sent_at < cutoff or not attempt.candidate_id:
                continue
            candidate = self.projections.candidates.get(attempt.candidate_id)
            if candidate is None:
                continue
    ...
    def _refresh_candidates(
        ...
        proposals = candidate_module.generate(
            ...
            boundaries=self.projections.boundaries.active(now),
            recent_events=self.events.read(EventQuery(limit=40, newest_first=True)),
            recently_spoken_memories=self._recently_spoken_memory_ids(now=now),
        )
