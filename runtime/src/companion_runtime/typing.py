...
class ValueProfile:
    ...
    The Chinese names come from the design document §4.2 and are *the* names for
    these axes -- do not re-translate them per file. Order here matches the
    document's example JSON exactly:

    | 轴 | 设计文档中文名 | 它决定什么 |
    |---|---|---|
    | ``autonomy`` | **自主倾向** | 自我推进的意愿 |
    | ``boundary_respect`` | **边界观念** | 对边界的敬畏、被拒绝时的收敛 |
    | ``emotional_expression`` | **情感表达** | 情绪有多直接地写进话里 |
    | ``relationship_maintenance`` | **关系维护** | 长期沉默后主动靠近的倾向 |
    | ``user_care`` | **用户关怀** | 被对方的未结之事推动的强度 |
    | ``conflict_directness`` | **冲突取向** | 把话挑明的倾向 |
    | ``stability_commitment`` | **稳定 / 承诺取向** | 不被单次波动带偏 |
    | ``curiosity`` | **好奇倾向** | 追问与了解的驱动 |

    Two traps worth keeping in view (both measured):

    * the *mechanical* meaning of ``autonomy`` is not the literal 自主: in
      :func:`~companion_runtime.emotion.appraise_event` it only scales emotional
      sensitivity by ``1 - 0.25 * autonomy``, i.e. a low value means "what I feel is
      not damped by self-containment", not "less self-directed";
    * a value only takes effect on rows that already exist when it is *read*:
      ``ValueProfile`` is written into ``runtime_state`` when an instance first
      creates its database (``Runtime._ensure_defaults``), so editing env/fleet
      config alone changes nothing for live instances -- see
      ``scripts/set_value_profile.sh``.
    """
...
class CandidateIntent:
    ...
    created_at: datetime | None = None
    updated_at: datetime | None = None
    #: When the *wording* (``intent``/``goal``/``constraints``) was last written. A row is
    #: touched far more often than its words change - a refresh merges numbers into a live
    #: candidate every round - and the render needs the words' own moment to read a
    #: relative phrase against ("中午" written at 11:44, spoken at 13:30).
    wording_at: datetime | None = None
    expires_at: datetime | None = None
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "created_at": _iso(self.created_at),
            "updated_at": _iso(self.updated_at),
            "wording_at": _iso(self.wording_at),
            "expires_at": _iso(self.expires_at),
