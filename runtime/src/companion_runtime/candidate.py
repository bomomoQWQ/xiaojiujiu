...
def _unfinished_candidate(
    ...
    return CandidateIntent(
        ...
        type="follow_up",
        # The matter title is cut from the user's own sentence, so it can carry the
        # moment that sentence was said. A draft must not (see DRAFT_TIME_WORDS).
...
def generate(
    *,
    ...
    recent_events: Sequence[RawEvent] = (),
    recently_spoken_memories: Sequence[str] = (),
    ...
    Args:
        ...
        recently_spoken_memories: Memory ids already used as a proactive opener inside
            ``candidate.memory_callback_cooldown_hours``. A memory in here must not
            produce another callback candidate: measured on the beta, one account whose
            whole material was a single "有呀有呀" received five proactive messages, four
            of them about that one memory, spread over a day and a half.
    ...
    shares = 0
    already_said = set(recently_spoken_memories)
    for activation, memory in activated[:4]:
        if memory.memory_id in already_said:
            # This one has already been the opening line inside the cooldown. The
            # per-candidate constraint tells the model "不要重复追问同一件事", but that is
            # text, and the ``repeat_cost`` window counts *contacts*, not material - so
            # before this guard a single activated memory could be re-opened every round
            # it stayed activated.
            continue
...
#: The vocabulary that anchors a draft's *relative* time. Kept as a table so the check is
#: the same everywhere, and so the render can be told *specifically* what to recompute.
#:
#: This module no longer forbids time in a draft, and no longer strips it: prohibiting it
#: destroyed the content ("明天中午" -> "") and the prompt was never a guarantee anyway.
#: What makes a relative phrase safe is that **code stamps it**: a draft carries when it
#: was written (``api_v1.RENDER_DRAFT_PREFIX``), every quoted utterance carries when it was
#: said (``deep_refresh._quoted``), and the render states both - so "中午" written at 11:44
#: and spoken at 13:30 is a checkable discrepancy rather than a silent lie. This table is
#: what says a draft contains one.
#:
#: The line is drawn at *anchor-dependent* time: a relative day ("明天"), a time of day
#: ("下午"), a week ("上周"), a clock ("三点") is only true at the moment it was said. A
#: calendar date ("生日是三月三号") is a fact about the user that stays true, and is not
#: flagged - it is not a moment, and treating it as one would corrupt the fact.
DRAFT_TIME_WORDS: tuple[str, ...] = (
    # 一天之内
    "凌晨",
    "清晨",
    "一大早",
...
#: Clock times. The numeral+点 form needs care: "有一点累" and "差一点" are not clock times,
#: so a bare "一点" only counts when no durational word precedes it. A calendar date is
#: deliberately *not* here: see the table's own note on where the line is drawn.
DRAFT_TIME_PATTERNS: tuple[re.Pattern[str], ...] = (
    re.compile(r"\d{1,2}\s*[:：]\s*\d{2}"),
    re.compile(
        r"(?<![有差这那多小几])"
        r"(?:[0-9]{1,2}|[一二三四五六七八九十两]{1,3})\s*[点時时](?:半|整|钟|[0-9]{1,2}\s*分)?"
    ),
)

#: Longest first, so "早上好" is removed as a greeting rather than leaving "好" behind.
_DRAFT_TIME_RE = re.compile(
    "|".join(re.escape(word) for word in sorted(DRAFT_TIME_WORDS, key=len, reverse=True))
)


def time_expression_in(text: str) -> str:
    """Return the time expression ``text`` names, or an empty string.

    The return value is the matched text rather than a flag so a rejection can say *what*
    tripped it - "明天下午三点" is a much more useful log line than "a draft named a moment".
    """
    body = text or ""
    match = _DRAFT_TIME_RE.search(body)
    if match:
        return match.group(0)
    for pattern in DRAFT_TIME_PATTERNS:
