...
#: The fields whose value *is* the draft's wording. A patch that touches one of these
#: rewrites what the character intends to say, and only then does the candidate's
#: ``updated_at`` move - see the update branch of :func:`apply_one`.
WRITING_FIELDS: frozenset[str] = frozenset({"intent", "goal", "constraints"})
...
def apply_one(
    ...
    *,
    ...
    if op == "update":
        ...
        # ``updated_at`` on a candidate means **when its wording was last written**, not
        # "when the pool last touched it". The distinction is load-bearing: a refresh
        # merges into a live candidate and updates its numbers every round while leaving
        # the words alone, so bumping the stamp unconditionally turned the draft's own
        # anchor (``api_v1._draft_written_at``) into "a moment ago" and the render could
        # no longer see how far the wording had drifted.
        if set(patch) & WRITING_FIELDS:
            candidate.updated_at = stamp
            candidate.wording_at = stamp
        projections.candidates.upsert(conn, candidate)
