...
def _fallback_block(bundle: Any) -> str:
    ...
    if time_context.get("local_now"):
        lines.append(f"- 当前本地时间：{time_context['local_now']}")
    lines.append("")
    lines.append("以上是仅本轮注入的临时背景，不要直接复述，也不要写进长期对话历史。")
    return "\n".join(lines)
...
def _render_payload(
    ...
    if block:
        lines.extend([block, ""])
    lines.append("【现在要写的话】")
    lines.append(f"- 想做的事：{intent or '主动联系对方'}")
    if goal:
