...
class UtilityConfig:
    ...
    #: Window for "how many times have I reached out lately": ``repeat_cost`` (bursting)
    #: and the drive's ``recent_contact_ratio`` both count inside it.
    #:
    #: Raised from 1h to 6h. At one hour the count never got past
    #: ``repeat_contact_tolerance``, so ``repeat_cost`` measured 0.000 across all 8,949
    #: utility rows of the beta and "the same thing again" had no price at all
    #: (repeated callbacks were 6-34 hours apart).
    #:
    #: Deliberately **not** 24-72h: the same count feeds ``recent_contact_ratio``, where
    #: two days of contact would read as "I have been pestering him" and suppress every
    #: normal opener. The long-horizon case - the same *memory* as an opener - belongs to
    #: ``candidate.memory_callback_cooldown_hours``, which is about material, not counts.
    repeat_window_seconds: float = 21600.0
...
class CandidateConfig:
    ...
    #: How long a memory that has already been used as a proactive opener keeps that
    #: door shut. Measured on the beta: one account whose entire material was a single
    #: "有呀有呀" got five proactive messages, four of them about that one memory (at
    #: +6h, +14h, +26h and +34h), because ``_memory_candidate`` regenerates from the
    #: same activated memory every round and nothing remembered that it had been said.
    #: The per-message constraint ("不要重复追问同一件事") is text for the model, and the
    #: ``repeat_cost`` window only counts *contacts* inside an hour, so neither could
    #: catch a callback repeated a day later. 48 hours covers "don't open with the same
    #: thing two days running"; 0 disables the guard.
    memory_callback_cooldown_hours: float = 48.0
