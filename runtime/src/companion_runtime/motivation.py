    ...
    *,
    ...
    for candidate in inputs.candidates:
        ...
        if hard_blocked and is_candidate_proactive(candidate):
            ...
            elif state.contact_count_today >= config.drive.max_contacts_per_day:
                # The day's budget is a calendar quantity. It used to be read from
                # `inputs.recent_contacts`, which counts contacts inside
                # `utility.repeat_window_seconds` - a *burst* window, not a day. With the
                # window at 6h the two agree often enough to hide the difference, but they
                # are not the same number, and `authorize.py` has always used
                # `contact_count_today` for this same gate (motivation.py:795 rolls the
                # day over before anything increments it).
                blocked = True
