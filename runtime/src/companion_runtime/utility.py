...
#: Weekday names for the readable local stamp. The host's own reminder spells them in
#: English ("Weekday: Friday"); every prompt of ours that states a moment uses this form,
#: so an anchor written by the code and a clock stated by the render read identically.
WEEKDAY_CN: tuple[str, ...] = ("周一", "周二", "周三", "周四", "周五", "周六", "周日")


def display_local(moment: datetime | None) -> str:
    """Return ``2026-11-14 19:36 周六（CST）``-style local wall time, or an empty string.

    One formatter for two jobs: the anchor attached to a quoted utterance ("he said this
    at …") and the clock a render states ("it is … now"). They must be readable the same
    way, or the model cannot compare them - and comparing them is the whole point: a
    relative phrase like "明天" is only legible against the moment it was written.
    """
    if moment is None:
        return ""
