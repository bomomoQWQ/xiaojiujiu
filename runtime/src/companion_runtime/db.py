...
class Database(DatabaseBase):
    ...
    ADDED_COLUMNS: tuple[tuple[str, str, str], ...] = (
        ...
        # When the draft's *wording* was last written, as opposed to when the row was last
        # touched. The two differ constantly: a refresh merges into a live candidate and
        # rewrites its numbers every round while leaving the words alone, and the render
        # needs the wording's own moment to see how far a draft has drifted ("中午" written
        # at 11:44, spoken at 13:30). Existing databases upgrade in place; old rows read
        # back as ``NULL``, and every reader then falls back to ``created_at`` - the same
        # thing, because before this column existed nothing could rewrite the wording
        # except an explicit update that also reset the row.
        ("candidate_intents", "wording_at", "TEXT"),
    )
