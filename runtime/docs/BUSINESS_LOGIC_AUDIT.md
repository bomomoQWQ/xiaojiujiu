# 主业务逻辑：三个已复现的缺陷（只诊断，未修）

> 结论先行，证据先行。**本文件记录问题与修法方向，代码一行未改**（用户指示"暂时不修"）。
> 所有数字都能用一条命令重新生成：
>
> ```bash
...
| | 缺陷 | 设计依据 | 用户可见后果 | 默认配置是否可达 |
|---|---|---|---|---|
| **A** | 回复长度用**绝对阈值** `<= 4` / `>= 20` 判证据强弱与正负 | §29 | 话少的用户被系统性判为冷淡：同样的行为，证据权重 **0.072 vs 0.180**，`positive_probability` **0.619 vs 0.700** | **是**（规则路径就会写 `reply_length`） |
| **B** | 硬边界用**自己那份 type 集合**判断"是否主动"，可被同义词绕过 | §52 / §86.5 | `repair` 被拦、`apology` 不被拦；`share` 被拦、`emotional_expression` 不被拦；`curious_question` 被拦、`question` 不被拦——**同一个意图换个写法就能绕过"今天别主动联系我"** | 否（provider 路径／`POST /candidates/operations`；打开 v0.2 的 `remote_api` 即生效） |
| **C** | 情绪时宜性同样按 type 硬编码，同义词被区别对待 | §29 / §7 情绪与行为的匹配 | 同一个意图（`repair` vs `apology`）时宜性 **1.000 vs 0.440**，差 **2.3 倍** | 否（同上） |
