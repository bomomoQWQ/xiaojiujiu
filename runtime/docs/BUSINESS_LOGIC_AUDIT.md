# 主业务逻辑：三个已复现的缺陷（B/A 已修，C 待修）

> **状态（2026-09-16 更新）**：**B 已修**（`94549a5`）、**A 已修**（含 §29 的第三句
> 对话持续长度）。**C 尚未修**。下面每条都标了当前状态；原始诊断文字保留原样，
> 便于对照"当初是怎么判的"。
>
...
|---|---|---|---|---|---|
| **A** | 回复长度用**绝对阈值** `<= 4` / `>= 20` 判证据强弱与正负；`min(3, turns)` 让对话长度对所有人都在 3 轮饱和 | §29 | 话少的用户被系统性判为冷淡：同样的行为，证据权重 **0.072 vs 0.180**，`positive_probability` **0.619 vs 0.700**；对话长度信息在 3 轮后全部丢失 | **是**（规则路径就会写 `reply_length`） | **已修** `a1d0b1e` |
...
| **C** | 情绪时宜性同样按 type 硬编码，同义词被区别对待 | §29 / §7 情绪与行为的匹配 | 同一个意图（`repair` vs `apology`）时宜性 **1.000 vs 0.440**，差 **2.3 倍** | 否（同上） | **待修** |
...
> **修法（`a1d0b1e`）**：照抄已有的 `reply_delay_baseline` 路径，加了 **两条** 同类基线：
>
...
## 4. 缺陷 C：情绪时宜性按 type 硬编码（§29 的另一面）
...
与 B 同一个修法：把"这个行为属于哪一类"收敛到 `TYPE_TO_BEHAVIOUR`，三处共用。
C 的映射比 B 细（它区分正/负情绪方向，不是简单的主动/被动），所以合理的做法是
按**行为类**给一组"合时宜性"规则，而不是按 type。这条需要设计判断：

- `repair` 只在负向时合时宜？`proactive_contact`（`contact`/`check_in`）呢？
- `emotional_expression` 在正向时合时宜吗？现在 `share` 是，`emotional_expression` 不是。

也就是说 C 的**集合本身**也需要一次确认，不能只做机械替换。
...
| **A** | `a1d0b1e` | 长度与对话轮数两条基线；两处绝对阈值替换；负向只能抵消加分 | `tests/test_reply_length_baseline.py` 12 条 + 8 变异 |
| **C** | 待修 | 情绪时宜性按行为类（集合本身需一次设计确认，见 §4） | — |

全仓变异数从 31 增至 **43，全部 KILLED**。
