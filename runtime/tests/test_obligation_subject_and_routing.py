...
class TestAProactivePromptStaysInItsChat:
    ...
    def test_the_render_block_omits_matters_from_other_chats(self, clock: SimulatedClock) -> None:
        ...
        try:
            # ``_context_text`` returns ``(block, clock)``: the clock is stated by the
            # render's instruction section and deliberately never inside the block.
            block_a, _clock_a = api_v1._context_text(
                runtime, BASE_TIME + timedelta(hours=1), conversation_id=SESSION_A
            )
            block_b, _clock_b = api_v1._context_text(
                runtime, BASE_TIME + timedelta(hours=1), conversation_id=SESSION_B
            )
    ...
    def test_an_unscoped_block_is_unchanged(self, clock: SimulatedClock) -> None:
        ...
        runtime, matter = self._matter_in_b(clock)
        try:
            block, _clock = api_v1._context_text(runtime, BASE_TIME + timedelta(hours=1))
