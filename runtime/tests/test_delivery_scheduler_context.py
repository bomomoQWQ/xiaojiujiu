...
def test_the_clock_is_local_wall_time_and_stays_out_of_the_injected_block(
    runtime: Runtime,
) -> None:
    """The clock must be local, and it must not be stated inside the background block.

    Two regressions live here. (1) ``build_time_context`` ran the local datetime through
    ``isoformat()``, which normalises to UTC, so a line labelled 当前本地时间 printed a UTC
    stamp. Chat turns survived it (the host appends its own correct reminder) but a
    proactive render carries no host reminder, so her proactive messages were written
    eight hours off - measured, a 04:01 CST render came out as 晚上好. (2) That clock was
    also rendered *inside* the injection block, and the block closes by saying it
    describes the state before this turn and is not how to react in it - so the only
    clock a render ever got arrived pre-de-authorised, and a 17:53 render still greeted
    the user with 早呀.

    The Runtime now states the time exactly once, in the render prompt's instruction
    section (``api_v1.RENDER_CLOCK_PREFIX``); code owns time, and a *duration* ("8 hours
    since") is cognition while a *clock* ("it is 17:53") is performance.
    """
    bundle = context_module.build(runtime=runtime, now=BASE_TIME)
    ...
    # The value a render states must be a clock, not a machine string.
    ...
    # The old defect in one assertion. Check the ISO *shape* - a ``T`` between the date
    # and the time - not the letter ``T`` anywhere in the value: the display ends with
    # the timezone abbreviation in parentheses, so a machine whose zone is CST / EST /
    # MST / PST failed a naive check while the behaviour was exactly right.
    assert f"{local:%Y-%m-%dT}" not in display, "the local value must not be an ISO stamp"

    # And the block must not carry it at all.
    block = context_module.render_block(bundle)
    assert "当前本地时间" not in block
    assert "- 现在是：" not in block


def test_the_fallback_block_states_no_clock_either(runtime: Runtime) -> None:
    """The *other* way a block gets built must obey the same rule.

    ``render_block`` returns empty when there is no psychological prose and no situation
    fact, and ``api_v1._fallback_block`` then repeats the preamble and the time section.
    It used to print ``local_now`` there - the ISO form, on a line labelled 当前本地时间 -
    so a deployment with no semantic provider walked straight back into the defect the
    clock removal was for. The duration stays: that is cognition, not a clock.
    """
    from companion_runtime import context as context_module
    from companion_runtime.api_v1 import _fallback_block

    bundle = context_module.build(runtime=runtime, now=BASE_TIME)
    bundle.psychological = {}          # no prose -> the renderer yields nothing
    bundle.situation["facts"] = []
    assert context_module.render_block(bundle) == ""

    fallback = _fallback_block(bundle)
