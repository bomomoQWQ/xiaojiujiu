...
它要做的事，一句话讲完：**让角色拥有持续的内在状态**——会记住、会重新理解、会形成想法、
会在没有外部命令时自己决定要不要联系你。**它不是一个每轮重新表演人格的聊天 Prompt。**
...
这一节讲**为什么这样设计**。每个模块具体更新什么、怎么算，属于运维手册的内容，
这里只讲决策与理由。
...
1. **它没有跨时间状态。** 上下文一换它就忘了。这不是提示词写得不够好，是机制上做不到。
2. **它不可审计。** 状态不存在于任何地方，就无法回答"它为什么这么说"。
3. **它很贵，而且更不稳。** 把每个认知问题都推给它，等于每轮都调一次大模型；
...
于是意图进入已提交状态，主 LLM 开始组织语言。就在这一刻，用户突然发来：

**"面试过啦！！"**

这里有个容易被忽略的设计：**已经决定要做、但还没发出去的动作，不能被新消息粗暴丢弃**。
Runtime 做的是并发重协调——它发现新消息恰好满足了原意图的目标，于是把那个意图
判成"目标已被满足"并合并，同时把"我刚决定要问、还没发出去"这件事写进给主 LLM 的
临时上下文里。

角色于是可以自然地说："你居然刚好发来了，我正准备问你呢。"
接下来：未尽之事了结、候选关闭、产生正向情绪事件、形成一次交互观察。**环闭合了。**
...
记在 `runtime/docs/BUSINESS_LOGIC_AUDIT.md`。
...
这套分工可以概括成四句话：**模型只负责语义，动力学交给代码，连续性交给数据库，
而"不发疯"交给协议层。**
