#!/usr/bin/env bash
# 查 qq02 在公告前后（11:50-12:10 CST）的全部收发，以及各容器真正停止的时刻。
set -u
...
echo
echo "=== 2) qq02 在 09-18 11:50 之后的全部收发（NapCat 日志，停着的容器也能读）==="
docker logs xxj-napcat-test 2>&1 | sed -E 's/\x1b\[[0-9;]*m//g' \
  | grep -a "(qq02)" | awk '$1=="09-18"' | awk '{ if ($2 >= "11:50:00") print }' | tail -40
