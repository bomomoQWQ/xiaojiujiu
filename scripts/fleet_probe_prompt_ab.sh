#!/usr/bin/env bash
# Which sentence in the shipped deep-refresh system prompt silences the model?
#
# Established: the shipped 315-char prompt yields six empty collections, while a
# 198-char paraphrase of it yields 2280 chars of real reinterpretations from the
# same payload. This walks the difference sentence by sentence.
#
# Usage:  bash scripts/fleet_probe_prompt_ab.sh
set -u
...
ROLE = (
    "\u524d\u4e94\u4e2a\u4e2d\u9664 psychological_interpretation \u662f\u5bf9\u8c61\u5916\uff0c"
    "\u5176\u4f59\u90fd\u662f\u6570\u7ec4\u3002"
    "\u4f60\u53ea\u63d0\u4f9b\u5efa\u8bae\uff0c\u4e0d\u51b3\u5b9a\u4efb\u4f55\u72b6\u6001\u53d8\u66f4\uff0c"
    "\u4e0d\u751f\u6210\u53f0\u8bcd\uff0c\u4e0d\u521b\u9020\u8f93\u5165\u4e2d\u4e0d\u5b58\u5728\u7684\u4e8b\u4ef6\uff1b"
)
CAUTION = "\u8bc1\u636e\u4e0d\u8db3\u65f6\u8fd4\u56de\u7a7a\u6570\u7ec4\u6216\u7a7a\u5bf9\u8c61\uff0c\u4e0d\u8981\u731c\u6d4b\u3002"

variants = {
    "shipped": DEEP_REFRESH_SYSTEM_PROMPT,
    "intro+fields": INTRO + FIELDS,
    "fields only": FIELDS,
    "intro+fields+role": INTRO + FIELDS + ROLE,
    "fields+caution": FIELDS + CAUTION,
    "no caution": INTRO + FIELDS + ROLE,
}
...
def call(label, system):
    ...
    print(f"[{label}] reply={len(text)} finish={choice.get('finish_reason')}")
