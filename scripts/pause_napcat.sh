#!/usr/bin/env bash
# 按用户要求：停 NapCat（切断 QQ 通道）。顺便查清那个陌生会话 qq07 是谁。
set -u
...
echo
echo "=== 4) 陌生会话 qq07 的来头（读 AstrBot 库，只读）==="
python3 - <<'PY'
import sqlite3
...
for table in tables:
    ...
    for col in ("unified_msg_origin", "umo", "session_id", "conversation_id", "sender_id"):
        ...
        try:
            hits = con.execute(
                "select count(*) from %s where cast(%s as text) like '%%qq07%%'"
        ...
        if hits:
            ...
            for row in con.execute(
                    "select %s from %s where cast(%s as text) like '%%qq07%%'"
