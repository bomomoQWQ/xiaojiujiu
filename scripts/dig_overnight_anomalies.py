"""深挖两件事：
 1) 1670681411 窗口内新建的 6 件事，是从哪些 source event 派生的？（若引用的正是"她自己刚发出的
    主动消息"，那就是 source 去重抓不到的那一类）
 2) 两个"从不开口"的实例（2206929446 / 2259606745）现在的 I/R/P 与 need —— 对照我之前那次
    过线推演，看模型哪里估错了。
"""
...
print()
print("########## 1) 1670681411：新事 vs 旧事，各自的 source 事件 ##########")
con = sqlite3.connect("file:/data/default-friendmessage-1670681411/companion.sqlite3?mode=ro",
                      uri=True)
...
for tag in ("2206929446", "2259606745"):
